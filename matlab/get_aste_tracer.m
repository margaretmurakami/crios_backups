...
%cc=strmatch(aa.class,'gcmfaces');
%if(bb==1);
if(strmatch(aa.class,'double'));
...
elseif(strmatch(aa.class,'gcmfaces'));
  [nx,tmp,nz]=size(F.f1);
  for k=1:5;
    if(nfx(k)>0&nfy(k)>0);
      eval(['tmp=F.f' num2str(k) ';']);
      if(k<=2);
        ff{k}=tmp(:,3*nx-nfy(k)+1:3*nx,:);
      else;
        ff{k}=tmp(1:nfx(k),:,:);
      end;
    else;
  ...
  error('wrong class of input, need to either be "double=compact" or "cell" or "gcmfaces=global" ');
end;
...
% mid left: face 3, rot 180deg
  ...
  iface=round([nfx(5)-nfy(5)*(1-fac1)+1,nfx(5)]);jface=max(1,round([(1-fac2)*nfy(5)-4,nfy(5)]));
  if(length(jaste(1):jaste(2))>length(jface(1):jface(2)));
    jaste(1)=jaste(2)-(length(jface(1):jface(2)))+1;
  end;
