...
try:
    ...
    class _MissingNumpy:
        def __getattr__(self, name: str) -> Any:
            raise ModuleNotFoundError("fullaste_run_config requires numpy for grid/run setup helpers.")
...
def find_data_diagnostics(run_dir: str) -> str:
    """
    Return the data.diagnostics path for a run directory.

    The notebooks use runs that may store namelists under either `namelists/`
    or `NAMELISTS/`. Pass the run root, e.g. `layers_path` or `dirrun`.
    """
    candidates = [
        os.path.join(run_dir, "namelists", "data.diagnostics"),
        os.path.join(run_dir, "NAMELISTS", "data.diagnostics"),
    ]
    ...
    raise FileNotFoundError(
        "Could not find data.diagnostics under namelists/ or NAMELISTS/ for "
        f"{run_dir!r}."
    )
...
def _clean_diag_name(name: str) -> str:
    return name.strip()


def parse_data_diagnostics(path: str) -> Dict[str, Any]:
    """
    Parse MITgcm `data.diagnostics` into stream and diagnostic lookup tables.

    Returns a dictionary with:
      - `streams`: stream number -> metadata dict with `filename`, `file_name`, `fields`
      - `diagnostic_to_file`: diagnostic field -> basename file prefix
      - `diagnostic_to_path`: diagnostic field -> path/prefix as written in namelist
      - `diagnostic_to_stream`: diagnostic field -> stream number

    Example:
        info = parse_data_diagnostics("namelists/data.diagnostics")
        info["diagnostic_to_file"]["ADVx_TH"] == "budg3d_hflux_set2"
    """
    with open(path, "r", encoding="utf-8") as f:
        text = "\n".join(_strip_namelist_comment(line) for line in f)

    filename_by_stream: Dict[int, str] = {}
    for match in re.finditer(r"filename\s*\(\s*(\d+)\s*\)\s*=\s*'([^']+)'", text, re.I):
        filename_by_stream[int(match.group(1))] = match.group(2).strip()

    fields_by_stream: Dict[int, list[str]] = {}
    field_re = re.compile(
        r"fields\s*\([^)]*,\s*(\d+)\s*\)\s*=\s*(.*?)(?=\n\s*[A-Za-z_]\w*\s*(?:\(|=)|\n\s*[&/]|$)",
        re.I | re.S,
    )
    for match in field_re.finditer(text):
        stream = int(match.group(1))
        fields = [_clean_diag_name(v) for v in re.findall(r"'([^']+)'", match.group(2))]
        fields_by_stream.setdefault(stream, []).extend([v for v in fields if v])

    streams: Dict[int, Dict[str, Any]] = {}
    diagnostic_to_file: Dict[str, str] = {}
    diagnostic_to_path: Dict[str, str] = {}
    diagnostic_to_stream: Dict[str, int] = {}

    for stream, filename in sorted(filename_by_stream.items()):
        fields = fields_by_stream.get(stream, [])
        ...
        for field in fields:
            diagnostic_to_file[field] = file_name
            diagnostic_to_path[field] = filename
            diagnostic_to_stream[field] = stream

    return {
        "path": path,
        "streams": streams,
        "diagnostic_to_file": diagnostic_to_file,
        "diagnostic_to_path": diagnostic_to_path,
        "diagnostic_to_stream": diagnostic_to_stream,
    }


def diagnostic_file_map(run_dir: str, *, basename: bool = True) -> Dict[str, str]:
    """
    Build `{diagnostic_name: file_prefix}` from a run's data.diagnostics.

    By default returns basename prefixes, which match notebook `file_name`
    usage with `rdmds(os.path.join(dirIn, file_name), ...)`.
    Set `basename=False` to preserve paths such as `diags/budg3d_hflux_set2`.
    """
    info = parse_data_diagnostics(find_data_diagnostics(run_dir))
    return info["diagnostic_to_file" if basename else "diagnostic_to_path"]


def diagnostic_file_name(run_dir: str, diagnostic: str, *, basename: bool = True) -> str:
    """Return the file prefix containing `diagnostic` for this run."""
    lookup = diagnostic_file_map(run_dir, basename=basename)
    key = _clean_diag_name(diagnostic)
    if key not in lookup:
        available = ", ".join(sorted(lookup)[:20])
        raise KeyError(
            f"Diagnostic {diagnostic!r} not found in {find_data_diagnostics(run_dir)}. "
            f"First available diagnostics: {available}"
        )
    return lookup[key]
